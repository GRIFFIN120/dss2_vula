<template>
  <div>
    ...
    <el-button type="success" size="small" plain @click="openOverview">区域总览</el-button>
          ...
          <template v-slot="{map}">
            <BaseBoundary :map="map" :style-param="6" :paths="borders" >
              <template v-slot="{border}">
                <BasePolygon v-for="item in areas" :key="item.id" :map="map" :style-param="item.polygonStyleId" :bean="item"  :show-label="false" :border="border" >
                </BasePolygon>
              </template>
            </BaseBoundary>
          </template>
    ...
    <BaseDialog ref="editorDialog" @confirm="mapConfirm" width="60%" title="编辑区域">
      <template v-slot="{data}">
        <BaseMap :StyleParam="1" :changeable="true">
          <template v-slot="{map}">
            <BaseBoundary :map="map" :style-param="6" :paths="borders" >
              <template v-slot="{border}">
                <BasePolygon ref="polygon" :map="map" :style-param="data.polygonStyleId" :bean="data"  :show-label="false" :border="border" >
    ...
    <BaseDialog ref="viewDialog"  width="60%" title="区域截图" :buttons="false" >
      <template v-slot="{data}" >
    ...
    <BaseDialog ref="textareaDialog" @confirm="textareaConfirm" width="60%" title="区域数据">
      <template v-slot="{data}">
    ...
    <BaseTable ref="table" :entity="entity" type="edit" :search="search">
      <template #operation="data">
        <!--  2. 添加按钮  -->
        <el-tooltip class="item" effect="light" content="编辑区域" placement="top">
          <el-button
            @click="openEditor(data.row)"
            size="mini" type="primary"  circle plain><i class="fa fa-clone"></i></el-button>
        </el-tooltip>

        <el-tooltip class="item" effect="light" content="区域数据" placement="top">
          <el-button
            @click="$refs.textareaDialog.open(data.row)"
            size="mini" type="primary"  circle plain><i class="fa fa-file-code-o"></i></el-button>
        </el-tooltip>

        <el-tooltip class="item" effect="light" content="区域截图" placement="top">
          <el-button
